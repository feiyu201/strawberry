...
namespace addons\autoapi\controller;
 ...
 * 
 */
class Autoapi extends AddonBase
{
    ...
    public function getList()
    {
        ...
        $count = Db::name('autoapi')->count();
        $data = Db::name('autoapi')->page($page, $limit)->select()->each(function ($item, $k) {
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            if (Db::name('autoapi')->where('id', $data['id'])->delete()) {
                self::add();
                $this->success("编辑成功");
            } else {
                $this->error("编辑失败");
        ...
        $wxappinfo = Db::name('autoapi')->where('id', $id)->find();
        if (!$wxappinfo) {
            $this->error("参数错误");
        }
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $res = (new \addons\autoapi\model\Autoapi())->where('table_name', $table)->count();
            if ($res) {
    ...
    protected static function annotate(&$data, $primary)
    {
        $str = '* @Apidoc\Param("'
            . $primary . '", '
            . 'type="int", ' . 'require=true, ' . 'desc="主键id")';
    ...
    public function delete()
    {
        ...
        if (Db::name('autoapi')->where('id', 'in', $idsStr)->delete()) {
            $this->success("删除成功");
        } else {
            $this->error("删除失败");
        }
    ...
    {
        $file_path = ADS_PATH . "autoapi/" . $name . ".txt";
    ...
    protected static function primary($primary)
    {
        $str = '* @Apidoc\Param("'
            . $primary . '", '
            .'type="int", ' . 'require=false, desc="主键")';
        return $str;
    }
    ...
    {
        $str = ($type ? "\r\n     " : "") . '* @Apidoc\Returned("' . $primary . '", type="int", ' . 'desc="id")';;
        foreach ($data as $elt => $v) {
            // 换行 且需要留5个空格
            $str = $str . "\r\n" . '     * @Apidoc\Returned("'
                . $v['field'] . '", '
