layui.define(['jquery', 'layer', 'laypage'], function(exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    ...
    Class.prototype.renderData = function(res, curr, count){
        ...
        layui.each(data,function(i,v){
            ...
            _content+='<li style="display:inline-block" data-type="'+_type+'" data-index="'+i+'">' +
                ...
                _img +
                '<p class="layui-elip" title="' + v.name + '">' + v.name + ' </p>' + '<div class="checked"></div><i class="layui-icon layui-icon-delete icon-img-delete" data-id="'+v.id+'"></i>' +
                '</div>' +
                '</li>';
        });
        options.elem.find('.fileManager').html(_content);
    ...
    Class.prototype.events = function(){
        ...
        //文件事件
        that.layBody.on('click', 'img', function(){ //单击行
            setPicEvent.call(this, 'pic');
        });
        that.layBody.on('click', 'i', function(){ //单击行
            setPicEvent.call(this, 'delete');
        });
        //文件夹事件
