    ...
    function (exports) {
        ...
        var pearAdmin = new function () {
            ...
            this.bodyRender = function (param) {
                ...
                var getInitUrlPromise = new Promise(function (resolve) {
                    resolve();
                });

                if (!window.initUrl && location.hash) {
                    window.initUrl = true;
                    ...
                    function getMenu(url, nodeArray) {
                        ...
                        for (let node of nodeArray) {
                            ...
                            if (node.children) {
                                for (let childNode of node.children) {
                                    var result = getMenu(url, childNode);
                                    if (result) {
                                        return result;
                                    }
                                }
                    ...
                    getInitUrlPromise = new Promise(function (resolve) {
                        getData(param.menu.data).then(function (menu) {
                            resolve(getMenu(url, menu));
                        });
                    });
                }

                getInitUrlPromise.then(function (data) {
                    console.log(data);
                    if (data) {
                        param.tab.index.title = data.title;
                        param.tab.index.id = data.id;
                        param.tab.index.href = data.href;
                    }
                    ...
                    if (isMuiltTab(param) === "true" || isMuiltTab(param) === true) {
                        ...
                        bodyTab.click(function (id) {
                            //EdwardCho
                            location.hash = $("[data-frameid='" + id + "']").attr("src");
                    ...
                    sideMenu.selectItem(param.tab.index.id);
                });
            }
