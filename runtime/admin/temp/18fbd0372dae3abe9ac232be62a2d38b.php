<?php /*a:1:{s:56:"E:\phpstudy\WWW\okadmin\app\admin\view\plugin\index.html";i:1600862206;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	...
	layui.use(["element", "jquery", "table", "form", "laydate", "okLayer", "okUtils", "okMock"], function () {
		...
		function uninstall(id) {
			okLayer.confirm("确定要卸载安装吗？", function () {
				okUtils.ajax("<?php echo url('uninstall'); ?>", "get", {id: id}, true).done(function (response) {
					if(response.code==1){
						console.log(response);
						//userTable.reload();
						//okUtils.tableSuccessMsg(response.msg);
		...
		function state(id) {
			okLayer.confirm("确定执行此操作吗？", function () {
				okUtils.ajax("<?php echo url('state'); ?>", "get", {id: id}, true).done(function (response) {
					if(response.code==1){
						console.log(response);
						//userTable.reload();
						//okUtils.tableSuccessMsg(response.msg);
