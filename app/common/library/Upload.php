...
class Upload
{
    ...
    {
        if (empty($file)) {
            throw new UploadException('No file upload or server upload limit exceeded');
        }

        $fileInfo = $_FILES['file'];
        $suffix = strtolower(pathinfo($fileInfo['name'], PATHINFO_EXTENSION));
        $suffix = $suffix && preg_match("/^[a-zA-Z0-9]+$/", $suffix) ? $suffix : 'file';
        $fileInfo['suffix'] = $suffix;//后缀
    ...
    protected function checkExecutable()
    {
        ...
        if (in_array($this->fileInfo['type'], ['text/x-php', 'text/html']) || in_array($this->fileInfo['suffix'], ['php', 'html', 'htm'])) {
            throw new UploadException('Uploaded file format is limited');
        }
        return true;
    }
    ...
    protected function checkImage($force = false)
    {
        ...
        if (in_array($this->fileInfo['type'], ['image/gif', 'image/jpg', 'image/jpeg', 'image/bmp', 'image/png', 'image/webp']) || in_array($this->fileInfo['suffix'], ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'webp'])) {
            ...
            if (!$imgInfo || !isset($imgInfo[0]) || !isset($imgInfo[1])) {
                throw new UploadException('Uploaded file is not a valid image');
            }
    ...
    protected function checkSize()
    {
        ...
        if ($this->fileInfo['size'] > $size) {
            throw new UploadException('File is too big (%sMiB). Max filesize: %sMiB.',
...
//        $md5 = $md5 ? $md5 : md5_file($this->fileInfo['tmp_name']);
        $md5 = $md5 ? $md5 : md5($this->fileInfo['tmp_name']);
        $replaceArr = [
            '{year}'     => date("Y"),
            '{mon}'      => date("m"),
            '{day}'      => date("d"),
            ...
            '{min}'      => date("i"),
            '{sec}'      => date("s"),
            '{random}'   => GetRandStr(16),
            '{random32}' => GetRandStr(32),
...
//        var_dump($filePath);
        ...
        if (!$completed) {
            $this->clean($chunkid);
            throw new UploadException('Chunk file info error');
        }
        ...
        if (!$destFile = @fopen($uploadPath, "wb")) {
            $this->clean($chunkid);
            throw new UploadException('Chunk file merge error');
        }
...
//        unset($file);
    ...
    public function chunk($chunkid, $chunkindex, $chunkcount, $chunkfilesize = null, $chunkfilename = null, $direct = false)
    {
        ...
        if (!move_uploaded_file($this->file->getPathname(), $destFile)) {
            throw new UploadException('Chunk file write error');
        }
...
//            }
            ...
            $sourceFile = $this->file['real_path'];
...
//            $file->setSaveName($fileName)->setUploadInfo($info);
        ...
        $this->file = $file;
        $add = [];
        $add['url'] = $storage . $fileName;
        $add['storage'] = $storage . $fileName;
        $add['filesize'] = $this->fileInfo['size'];
        $add['mimetype'] = $this->fileInfo['suffix'];
        $add['sha1'] = $sha1;
        $add['createtime'] = time();
        $add['updatetime'] = time();
        $add['uploadtime'] = time();
        $add['imagewidth'] = $this->fileInfo['imagewidth'];
        $add['imageheight'] = $this->fileInfo['imageheight'];
        $add['imagetype'] = $this->fileInfo['type'];
        $res = Db::name('attachment')->insert($add);

//        \think\Hook::listen("upload_after", $add);
        return $res;
    }
